...
- Exercise layout, buttons, rows of separate answer buttons (`group`), segmented radio choices (`choices`/`choice`) and slider (track, selector, hit/miss result, marker) are shared in `src/exercises/exercise.module.css`; an exercise module only adds what is specific to it.
...
4. Its component takes `ExerciseProps` (`frontend/src/exercises/Series.tsx`) and only renders the question UI, for the given `level` (no level picker). When the answer is submitted (by its Check button, or by clicking an answer button when a single click is the whole answer) it reports `{ label, isHit }` through `onCheck` (label: the target, as in its feedback text); its Next button calls `onNext` and reads "See score" when `isLastQuestion` (then it doesn't start a new question). On mount it focuses its first answer control. Rows of answer buttons also answer by position from the keyboard, with `answerIndexOf`/`ANSWER_KEYS` (`frontend/src/exercises/answerKeys.ts`: 1–9, 0, -, =, matched by physical key so any layout works). Each button shows its key in an `aria-hidden` `<kbd className={shared.key}>` and declares it with `aria-keyshortcuts`. When `isAutoPlay`, each new question plays without the Play button: the first one on mount, the next ones from Next. It can change during the exercise, so Next reads its current value.
5. Map its id to its component in `exercisePages` (`frontend/src/App.tsx`). `Series` renders it below the exercise name heading and owns the level choice, the auto-play option (shown next to the level, changeable at any time), the mode (free practice or scored series of `SERIES_LENGTH` questions), progress, score and results.
