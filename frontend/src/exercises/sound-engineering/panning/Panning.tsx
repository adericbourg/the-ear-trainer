...
export default function Panning({ level, isLastQuestion, isAutoPlay, onCheck, onNext }: ExerciseProps) {
  ...
  const reset = () => {
    ...
    setIsChecked(false)
    if (isAutoPlay) startNoise(newTarget)
  }
  ...
  const autoPlay = useEffectEvent(() => {
    if (isAutoPlay) startNoise(target)
  })
